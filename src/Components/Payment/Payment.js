...
const Payment = ({ details, setOpen, paymentStatus }) => {
    ...
    const payment = (mode) => {
        Axios.post('/api/v1/app/services/postOrder', {
            productId: details.product_id,
            mode: mode,
            address: address,
            userId: user.user_id,
        })
            .then(res => { console.log(res); paymentStatus(true); })
            .catch(err => { console.log(err); paymentStatus(false); })
    }
